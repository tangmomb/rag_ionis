...
def build_chunks_payload(text, speakers):
...
def create_chunks(video_path, force=False):
    ...
    if speakers is None:
        ...
        return None
    if target.exists() and not force and output_is_current(target, (source, speakers_source)):
        print(f"[skip] {target.name} existe deja")
        return target
    if target.exists() and not force:
        print(f"[regen] {target.name}: transcript ou speakers plus recents")
    normalized = normalize_text(source.read_text(encoding="utf-8"))
    if not normalized:
        print(f"[skip] transcript vide: {source}")
        return None
    payload = build_chunks_payload(normalized, speakers)
...
def main():
    ...
    done = sum(bool(create_chunks(video_path, force=args.force)) for video_path in videos)
