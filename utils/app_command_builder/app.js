...
const actions = [
  ...
  },
  {
    ...
    short: "Reconstruire updater et synchroniser YouTube",
    description: "À lancer après la mise à jour Git : reconstruit l’image temporaire updater, synchronise les statistiques et les commentaires YouTube, puis supprime le conteneur à la fin.",
    icon: "↻",
    accent: "#d9eee8",
    shellCommand: "cd rag_ionis/ && docker compose -f docker-compose.yml -f docker-compose.prod.yml --env-file .env.production --profile jobs run --rm --build updater python -m pipeline.update_stats",
    shellLabel: "Commande Linux VPS",
    terminal: { label: "Linux · VPS", prompt: "ubuntu@vps:~$" },
    sections: [],
  },
  {
    ...
    short: "Reconstruire updater et traiter les nouvelles vidéos",
    description: "À lancer après la mise à jour Git : reconstruit l’image temporaire updater, détecte les vidéos YouTube absentes de PostgreSQL et lance leur traitement, puis supprime le conteneur à la fin.",
    icon: "↻",
    accent: "#dce8f5",
    shellCommand: "cd rag_ionis/ && docker compose -f docker-compose.yml -f docker-compose.prod.yml --env-file .env.production --profile jobs run --rm --build updater python -m pipeline.update_videos",
    shellLabel: "Commande Linux VPS",
    terminal: { label: "Linux · VPS", prompt: "ubuntu@vps:~$" },
    sections: [],
  },
